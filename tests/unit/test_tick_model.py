...
class TestTickModel:
    ...
    def test_float32_precision(self):
        """Float32精度への変換テスト"""
        # 高精度の値を入力（Float32でも区別できる値）
        tick = Tick(
            timestamp=datetime.now(),
            symbol="EURUSD",
            bid=1.23456,
            ask=1.23466,  # Float32でも区別できる差
    ...
    def test_spread_calculation(self):
        ...
        # Float32精度での期待値を計算
        import numpy as np
        expected_spread = float(np.float32(1.12355) - np.float32(1.12345))
    ...
    def test_invalid_spread(self):
        ...
        errors = exc_info.value.errors()
        assert any("Ask price must be greater than bid price" in str(error.get('msg', '')) for error in errors)
