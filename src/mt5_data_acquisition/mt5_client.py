...
class MT5ConnectionManager:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self._max_retries: int = self._get_config_value(
            "max_retries", self.DEFAULT_MAX_RETRIES
        )
        self._retry_delay: int = self._get_config_value(
    ...
    def connect(self, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self._terminal_info:
                self.logger.info(
                    f"ターミナル情報: 会社={self._terminal_info.company}, ビルド={self._terminal_info.build}"
                )
            ...
            if self._account_info:
                self.logger.info(
                    f"アカウント情報: ログイン={self._account_info.login}, 残高={self._account_info.balance}, レバレッジ={self._account_info.leverage}"
                )
    ...
    def disconnect(self) -> None:
        ...
        if mt5 is None:
            self.logger.warning(
                "MetaTrader5パッケージがインストールされていないため、切断処理をスキップします"
            )
            return
    ...
    def reconnect(self) -> bool:
        ...
        for attempt in range(1, self._max_retries + 1):
            ...
            # 再接続試行のログ
            self.logger.info(
                f"再接続を試行します... (試行回数: {attempt}/{self._max_retries})"
            )
    ...
    def _increment_retry_count(self) -> int:
        ...
        self._retry_count += 1
        self.logger.debug(
            f"再試行カウンタをインクリメント: {self._retry_count}/{self._max_retries}"
        )
        return self._retry_count
    ...
    def _calculate_backoff_delay(self) -> float:
        ...
        self.logger.debug(
            f"バックオフ待機時間を計算: {delay}秒 (試行回数: {self._retry_count})"
        )
        return delay
...
class ConnectionPool:
    ...
    def __init__(self, config: dict[str, Any], max_connections: int = None):
        ...
        self.logger.info(
            f"接続プールを初期化しました (最大接続数: {self._max_connections})"
        )

    def _create_connection(self) -> MT5ConnectionManager | None:
        """新しい接続を作成
    ...
    def acquire(self, timeout: float | None = None) -> MT5ConnectionManager | None:
        """利用可能な接続を取得
            ...
            while True:
                ...
                if self._idle_connections:
                    ...
                    if connection.is_connected():
                        ...
                        self.logger.debug(
                            f"アイドルプールから接続を取得 (アクティブ: {len(self._active_connections)}, アイドル: {len(self._idle_connections)})"
                        )
                        return connection
                    else:
                        # 接続が切れている場合は新しく作成を試みる
                        self.logger.warning(
                            "アイドル接続が切断されていました。再作成を試みます"
                        )
                        continue

                # 新規作成可能か確認
                total_connections = len(self._active_connections) + len(
                ...
                if total_connections < self._max_connections:
                    ...
                    if connection:
                        self._active_connections.append(connection)
                        self.logger.debug(
                            f"新規接続を作成してアクティブに追加 (アクティブ: {len(self._active_connections)}, アイドル: {len(self._idle_connections)})"
                        )
                        return connection
                    else:
                ...
                # 最大接続数に達している場合は待機
                self.logger.debug(
                    f"最大接続数に達しています。待機中... (最大: {self._max_connections})"
                )
        ...
        with self._connection_available:
            ...
            if connection in self._active_connections:
                ...
                if connection.is_connected() and not self._closed:
                    ...
                    self.logger.debug(
                        f"接続をアイドルプールに返却 (アクティブ: {len(self._active_connections)}, アイドル: {len(self._idle_connections)})"
                    )
                    # 待機中のスレッドに通知
    ...
    def close_all(self) -> None:
        ...
        with self._lock:
            ...
            self.logger.info(
                f"接続プールをクローズしました (切断した接続数: {total_closed})"
            )
    ...
    def get_status(self) -> dict[str, int]:
        ...
        with self._lock:
            ...
            self.logger.debug(
                f"プール状態: アクティブ={status['active']}, アイドル={status['idle']}, 最大={status['max_connections']}"
            )
            return status
...
class HealthChecker:
    ...
    def __init__(
        ...
        self.logger.info(
            f"HealthChecker初期化: interval={interval}秒, auto_reconnect={auto_reconnect}"
        )
    ...
    def _check_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self.logger.error(
                    f"ヘルスチェックループでエラーが発生: {str(e)}", exc_info=True
                )
                # エラーが発生しても継続
    ...
    def _perform_check(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"ヘルスチェック実行中にエラーが発生: {str(e)}", exc_info=True
            )
