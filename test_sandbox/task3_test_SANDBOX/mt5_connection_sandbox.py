...
def main():
    ...
    # 接続設定（実際の値に置き換えてください）
    config = {
        'account': 12345678,  # アカウント番号
        'password': 'your_password',  # パスワード
        'server': 'YourBroker-Demo',  # サーバー名
        'timeout': 60000,  # タイムアウト（ミリ秒）
        # 'path': 'C:\\Program Files\\MetaTrader 5\\terminal64.exe'  # MT5のパス（オプション）
    }
    ...
    try:
        # 1. MT5への接続
        logger.info("=== MT5接続テスト開始 ===")
        
        if not sandbox.connect_to_mt5(config):
