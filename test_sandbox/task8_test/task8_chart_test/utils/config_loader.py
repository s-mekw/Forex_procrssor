...
class Task8Config:
    ...
    def symbol(self) -> str:
        ...
        return os.environ.get('TASK8_SYMBOL', self.chart.symbol)
    
    @property
    def timeframe(self) -> str:
        """現在のタイムフレームを取得"""
        return os.environ.get('TASK8_TIMEFRAME', self.chart.timeframe)
    
    @property
    def ema_periods(self) -> List[int]:
        """EMA期間リストを取得"""
        env_periods = os.environ.get('TASK8_EMA_PERIODS')
        if env_periods:
...
def load_config(
    ...
    if config_file is None:
        config_file = Path(__file__).parent.parent / "task8_config.toml"
    ...
    # 環境変数でオーバーライド
    if os.environ.get('TASK8_SYMBOL'):
        config_data["chart"]["symbol"] = os.environ['TASK8_SYMBOL']
    if os.environ.get('TASK8_TIMEFRAME'):
        config_data["chart"]["timeframe"] = os.environ['TASK8_TIMEFRAME']
    if os.environ.get('TASK8_TEST_MODE'):
        config_data["general"]["test_mode"] = os.environ['TASK8_TEST_MODE']
...
def validate_config(config: Task8Config) -> bool:
    """
    設定の妥当性を検証
...
def print_config(config: Task8Config, show_source: bool = False):
    """
    設定を見やすく表示
    ...
    # 基本設定テーブル
    table = Table(title="Task 8 Configuration", show_header=True)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Task 8 E2E Test Configuration')
    parser.add_argument('--config', type=str, help='Configuration file path')
    parser.add_argument('--preset', type=str, choices=['quick', 'full', 'performance'],
                       help='Preset configuration')
    parser.add_argument('--symbol', type=str, help='Trading symbol (e.g., BTCUSD#)')
...
def apply_args_to_env(args):
    """コマンドライン引数を環境変数に設定"""
    if args.symbol:
        os.environ['TASK8_SYMBOL'] = args.symbol
    if args.timeframe:
        os.environ['TASK8_TIMEFRAME'] = args.timeframe
    if args.ema_periods:
        os.environ['TASK8_EMA_PERIODS'] = args.ema_periods
    if args.test_mode:
        os.environ['TASK8_TEST_MODE'] = args.test_mode
